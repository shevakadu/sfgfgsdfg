...
//			})
			...
			bar.header.onclick = function() {
				...
				if (clickedon.open == false) {
					clickedon.open = true
					clickedon.contents.content_source.style.height = clickedon.initial_height
					clickedon.contents.content_source.style.padding = "20px"
				} else {
					clickedon.open = false
					clickedon.contents.content_source.style.height = "0px"
					clickedon.contents.content_source.style.padding = "0px"
				}
			}
		}
	}
...
//		console.log("bye:(!!111!1!!!1")
	...
	function highlight_search_option(option) {
		...
		document.getElementById("searchbar-input").setAttribute("placeholder", {"look-for-a-user": "Enter a username to see their posts and info.", "look-for-a-tag": "Type in a tag to find posts with that tag.", "look-for-a-mention": "See posts which mention someone."}[highlitedsearchoption])
	}
	highlight_search_option("look-for-a-user")
	console.log("search options everybody")
	console.log(searchoptions)
	
	const translation = {
		"userbar-welcome": `Добро пожаловать в нашу социальную сеть!<br>
				Если у вас есть код приглашения, вы можете присоединиться к нашему сообществу.<br>
				Не стесняйтесь исследовать и общаться с теми, кто разделяет ваши интересы.<br><break></break>
				Примечание: Используйте форму Аутентификация снизу чтобы зарегистрироваться или войти.`
	}
